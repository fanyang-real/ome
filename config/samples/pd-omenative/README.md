...
| Annotation | Declared on | Expected outcome in the merged Component metadata |
|---|---|---|
| `example.com/owner` | service only | present on **both** engine and decoder |
| `example.com/tier` | service **and** each Component | Component value wins (`engine-level` / `decoder-level`) |
| `example.com/slice-hint` | service only | survives, so a scheduler can read it |

Where that merged metadata then lands depends on the mode, so the two samples are
checked in different places:

| Mode | Merged metadata lands on | Observable as |
|---|---|---|
| `RawDeployment`, `MultiNode` | the Deployment / LeaderWorkerSet pod template | annotations on running pods (`30-inferenceservice-gang.yaml`) |
| `OMENative` | `InferenceReplica` `spec.runners[].template.metadata` | annotations on the InferenceReplica; **no pods in this build** (`20-inferenceservice.yaml`) |
